...
class HeiferGenerator:
	...
	def get_file_cows():
		if HeiferGenerator.file_cows is None:
			...
			for filename in os.listdir('/Users/kylepartridge/Desktop/CowLab/Lab7/cows'):		# for zybooks, replace 'cows' with '.'
				if HeiferGenerator.filter(filename):
					# for zybooks, replace os.path.join('cows', filename) with filename
					c = FileCow(filename[0:-4], os.path.join('/Users/kylepartridge/Desktop/CowLab/Lab7/cows', filename))
